...
int HdfsColumnarScanner::TransferScratchTuples(RowBatch* dst_batch, std::bitset<1024>& selected_rows) {
  ...
  const int num_rows_to_commit = ProcessScratchBatchCodegenOrInterpret(dst_batch, selected_rows);
...
int HdfsColumnarScanner::ProcessScratchBatchCodegenOrInterpret(RowBatch* dst_batch, std::bitset<1024>& selected_rows) {
