...
vector<SlotId> HdfsParquetScanner::GetSlotIdsForConjuncts(vector<ScalarExpr*> conjuncts) {
    ...
    if (filter_readers.empty() || non_filter_readers.empty() || materialization_threshold_ < 0) {
      ...
      if (num_row_to_commit == 0) {
        ...
        Status fill_status = FillScratchBatch(non_filter_readers, row_batch,
            ...
            num_micro_batches, scratch_batch_->num_tuples, num_tuples);
        if (!fill_status.ok() || *skip_row_group) {
          return fill_status;
        }
      }
...
Status HdfsParquetScanner::FillScratchBatch(
    ...
    for (int r = 0; r < num_micro_batches; r++) {
      ...
      if (col_reader->max_rep_level() > 0) {
        continue_execution = col_reader->ReadValueBatch(&scratch_batch_->aux_mem_pool,
            micro_batches[r].length, tuple_byte_size_, tuple_mem, &num_tuples);
      } else {
        continue_execution =
