...
Status GroupingAggregator::ProcessRow(
  ...
  Tuple* tuple;
  HashTable::Iterator it = hash_tbl->FindBuildRowBucket<false>(ht_ctx, &found, &tuple);
  ...
  if (AGGREGATED_ROWS) {
    ...
    UpdateTuple(dst_partition->agg_fn_evals.data(), tuple, row);
    return Status::OK();
  }
...
bool GroupingAggregator::TryAddToHashTable(HashTableCtx* __restrict__ ht_ctx,
  ...
  HashTable::Iterator it = hash_tbl->FindBuildRowBucket<false>(ht_ctx, &found, &intermediate_tuple);
  if (!found) {
