...
class StringParser {
 ...
 private:
  ...
  static inline T StringToFloatInternal(const char* s, int len, ParseResult* result) {
    ...
    // UNDERFLOW error.
    double val = 1;
    if (fast_double_parser::is_integer(s[len]) || *(s + i) == '.') {
      // Need to make copy of a string which is null-terminated for the parser.
      ...
      // String copy is not required
      s_end = fast_double_parser::parse_number(StripLeadingZeros(s + i, len - i), &val);
      trailing_len = (int) (s + len - s_end);
    }
